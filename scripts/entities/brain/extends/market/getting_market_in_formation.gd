extends BTSequence

func get_utility_score(_actor :Node) -> float:
	var agent :Villagent = _actor
	var blackboard :Blackboard = %Blackboard
	if not get_tree().get_first_node_in_group("Market"):
		return 0.0
	if agent.home == null:
		return 0.0
	if agent.home.house_level < 1:
		return 0.0
	if agent.market_id == 0:
		return 0.0
	if not agent.role.has(Enums.WorkType.SellResource) and not agent.role.has(Enums.WorkType.BuyResource):
		return 0.0
	if blackboard.board.has("market_updated"):
		if blackboard.board["market_updated"]:
			return 0.0
	
	
	return 0.95
